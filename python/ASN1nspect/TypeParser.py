...
class TypeParser:
	...
	def parse_predefined_types(self) -> None:
		try:
			predefined_types = resources.read_text("ASN1nspect.data", "types.cpp")
	...
	def __get_skeletons(self) -> List[str]:
		skeletons = []
		try:
			skeleton_path = resources.files("ASN1nspect.data.skeletons")
	...
	def parse_folder_types(self) -> None:
		...
		try:
			data_path = resources.files("ASN1nspect.data.code")
